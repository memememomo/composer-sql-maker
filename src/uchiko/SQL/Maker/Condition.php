...
class Condition {
    ...
    public function quote($label) {
        ...
        return
            Util::quoteIdentifier($label, $this->quote_char, $this->name_sep);
    }

    private function makeTerm($col, $val) {
        if (is_object($val) && ! Util::is_scalar($val)) {
            $self = $this;
        ...
        if ( is_array( $val ) &&  Util::is_hash( $val ) ) {
            foreach ($val as $op => $v) { }

            if ( (strcmp($op, 'IN') === 0 || strcmp($op, 'NOT IN') === 0) && is_array($v) && !Util::is_hash($v) ) {
                if ( count($v) === 0 ) {
                    if ( strcmp($op, 'IN') === 0 ) {
            ...
            else if ( ( strcmp($op, 'BETWEEN') === 0 ) && is_array($v) && !Util::is_hash($v) ) {
                if ( count($v) !== 2 ) {
                    throw new Exception("USAGE: make_term('foo', array('BETWEEN => array(Maker::scalar($a), Maker::scalar($b)))");
                }
            ...
            if ( is_array($v) ) {
                ...
                // make_term('foo', Maker::scalar("> 3") => foo > 3
...
IN:        array('foo', \uchiko\SQL\Maker::scalar('IS NOT NULL'))
...
IN:        array('foo_id', \uchiko\SQL\Maker::scalar(array('IN (SELECT foo_id FROM bar WHERE t=?)',44)))
OUT QUERY: '`foo_id` IN (SELECT foo_id FROM bar WHERE t=?)'
OUT BIND:  array('44')

IN:        array('foo_id', array('IN' => \uchiko\SQL\Maker::scalar(array('SELECT foo_id FROM bar WHERE t=?',44))))
OUT QUERY: '`foo_id` IN (SELECT foo_id FROM bar WHERE t=?)'
OUT BIND:  array('44')

IN:        array('foo_id', \uchiko\SQL\Maker::scalar(array('MATCH (col1, col2) AGAINST (?)','apples')))
...
IN:        array('created_on', array('>' => \uchiko\SQL\Maker::scalar('DATE_SUB(NOW(), INTERVAL 1 DAY)')))
