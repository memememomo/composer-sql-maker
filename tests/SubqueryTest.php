...
class SubqueryTest extends PHPUnit_Framework_TestCase {
    ...
    public function testSubqueryAndJoin() {
        $subquery = new Select(array( 'quote_char' => '', 'name_sep' => '.', 'new_line' => ' '));
        $subquery->addSelect('*');
        $subquery->addFrom( 'foo' );
        $subquery->addWhere('hoge', 'fuga');

        $stmt = new Select(array( 'quote_char' => '', 'name_sep' => '.', 'new_line' => ' '));
    ...
    public function testComplex() {
        $s1 = new Select(array( 'quote_char' => '', 'name_sep' => '.', 'new_line' => ' '));
        $s1->addSelect('*');
        $s1->addFrom( 'foo' );
        $s1->addWhere('hoge', 'fuga');

        $s2 = new Select(array( 'quote_char' => '', 'name_sep' => '.', 'new_line' => ' '));
        $s2->addSelect('*');
        $s2->addFrom( $s1, 'f' );
